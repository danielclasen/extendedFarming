...
public class PacketHandler implements IPacketHandler {
	...
	private void handleParticle(Packet250CustomPayload packet, Player player) {
		...
		if (particleType.equals("rain")){
			...
			FMLClientHandler.instance().getClient().effectRenderer.addEffect(new EntityRainFX(worldObj, posX, posY, posZ));
		...
		worldObj.markBlockForUpdate(new Double(posX).intValue(), new Double(posY).intValue(), new Double(posZ).intValue());
